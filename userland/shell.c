...
static int
PrepareArguments(char *line, char **argv, unsigned argvSize, int *join)
{
...
main(void)
{
    ...
    for (;;) {
        ...
        if (PrepareArguments(line, argv, MAX_ARG_COUNT, &join) == 0) {
        ...
        const SpaceId newProc = join ? Exec(line, argv,1) : Exec(line+1, argv,0);
