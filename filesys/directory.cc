...
{
    DEBUG('F', "thread %d, fetchFrom archivo %s \n", currentThread->myId, file->fileName);
    ASSERT(file != nullptr);
...
void
Directory::List(int ntabs) const
{
    for (unsigned i = 0; i < raw.tableSize; i++)
        if (raw.table[i].inUse){
            for(int j=0;j<ntabs;j++) printf("\t");
            printf("%s\n", raw.table[i].name);
            if (raw.table[i].isDir){
                Directory* subdir= new Directory(1);
                OpenFile* osubdir= new OpenFile(raw.table[i].sector,raw.table[i].name);
                listOpenFiles->Add(raw.table[i].name);
                subdir->FetchFrom(osubdir);
                subdir->List(ntabs+1);
                listOpenFiles->Remove(raw.table[i].name);
                delete subdir;
                delete osubdir;
            }
        }
...
Directory::Print() const
{
    ...
    for (unsigned i = 0; i < raw.tableSize; i++)
        if (raw.table[i].inUse) {
            ...
            hdr->Print(nullptr);
            if (raw.table[i].isDir) {
                Directory *child = new Directory(1);
                OpenFile *childOf = new OpenFile(raw.table[i].sector, raw.table[i].name);
                listOpenFiles->Add(raw.table[i].name);
                child->FetchFrom(childOf);
                printf("\n-----Directorio %s-----\n", raw.table[i].name);
                child->Print();
                delete child;
                delete childOf;
...
{
    for(unsigned i=0; i<raw.tableSize; i++) if (raw.table[i].inUse) return false;
    return true;
}
