...
public:
    ...
    /// Alocar espacio en el disco para cuando lo que se aloco en el Create
    /// no alcanza.
    bool UpdateRaw(Bitmap *freeMap, unsigned size);

    void AddIndir(unsigned sector_);
    ...
    int sector;//Deberiamos hacerlos metodos

    FileHeader *next = nullptr;
private:
    RawFileHeader raw;//se reserva el ultimo lugar del array para guardar el numero de sector de un
    //posible fileheader next
    //numSectors y numBytes no tiene en cuenta la informacion del ultimo lugar del array reservado para el next
};
