...
/// * `-t`  -- test nuestros para probar file system de Nachos.
///
/// *NETWORK* options
/// -----------------
///
...
main(int argc, char **argv)
{
    ...
    for (argc--, argv++; argc > 0; argc -= argCount, argv += argCount) {
        ...
        if (!strcmp(*argv, "-cp")) {         // Copy from UNIX to Nachos.
            ...
            fileSystem->List();
            printf("\n");
        } else if (!strcmp(*argv, "-mkdir")) { // Crear directorio 
            ASSERT(argc > 1);
            fileSystem->CreateDir(*(argv + 1));
            printf("\n");
        } else if (!strcmp(*argv, "-cd")) { // Cambiar de directorio 
            ASSERT(argc > 1);
            ...
            PerformanceTest();
          else if (!strcmp(*argv, "-t")) // Nuestros tests.
            Tests();
