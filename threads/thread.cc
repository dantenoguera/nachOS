...
Thread::Thread(const char *threadName, bool join, int p)
{
    ...
    openFiles->Add(nullptr);
    myId=spaceIds->Add(this);
    ASSERT(myId!=-1);
#endif

    joinable = join;
    if(joinable) waitChild = new Channel(name);
    prio = p;
}
...
Thread::~Thread()
{
    ...
    if(joinable) delete waitChild;

    #ifdef USER_PROGRAM
        delete openFiles;
        delete space;
        spaceIds->Remove(myId);
    #endif
}
...
Thread::Finish(int ret)
{
    ...
    if (joinable) waitChild->Send(ret);
...
int
Thread::Join(){
    DEBUG('t', "Thread `%s` esperando. \n", currentThread->GetName());
    ASSERT(joinable);
    int mensaje;
    waitChild->Receive(&mensaje);
    DEBUG('t', "Thread `%s` reanudado. \n", currentThread->GetName());
    return mensaje;
}

void
Thread::EditPriority(int p){
    prio=p;
}
